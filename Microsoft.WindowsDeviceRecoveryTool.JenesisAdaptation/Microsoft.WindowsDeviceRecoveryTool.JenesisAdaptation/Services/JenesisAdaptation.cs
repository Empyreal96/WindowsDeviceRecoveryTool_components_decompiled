...
namespace Microsoft.WindowsDeviceRecoveryTool.JenesisAdaptation.Services
{
	// Token: 0x02000006 RID: 6
	[PartCreationPolicy(CreationPolicy.Shared)]
	[ExportAdaptation(Type = PhoneTypes.Jenesis)]
	public class JenesisAdaptation : BaseAdaptation
	{
		// Token: 0x06000011 RID: 17 RVA: 0x00002520 File Offset: 0x00000720
		[ImportingConstructor]
		public JenesisAdaptation(FfuFileInfoService ffuFileInfoService, MsrService msrService, ReportingService reportingService)
		{
		...
		public override PackageFileInfo CheckLatestPackage(Phone phone, CancellationToken cancellationToken)
		{
			...
			try
			{
				Tracer<JenesisAdaptation>.LogEntry("CheckLatestPackage");
			...
			{
				Tracer<JenesisAdaptation>.LogExit("CheckLatestPackage");
			}
			return result;
		}
		...
		{
			Tracer<JenesisAdaptation>.LogEntry("CheckPackageIntegrity");
			this.ffuFileInfoService.ReadFfuFile(phone.PackageFilePath);
			Tracer<JenesisAdaptation>.LogExit("CheckPackageIntegrity");
		}
		...
		{
			Tracer<JenesisAdaptation>.LogEntry("DownloadPackage");
			...
			try
			{
				...
				DownloadParameters downloadParameters = new DownloadParameters
				{
					...
					DestinationFolder = ((!string.IsNullOrEmpty(phone.QueryParameters.ManufacturerHardwareModel)) ? Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetJenesisProdcutsPath(phone.QueryParameters.ManufacturerHardwareModel) : Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetJenesisProdcutsPath(phone.QueryParameters.ManufacturerHardwareVariant)),
					FilesVersioned = true
				};
				Tracer<JenesisAdaptation>.WriteInformation("Download Params: {0}", new object[]
				{
					downloadParameters
				});
			...
			catch (Exception ex)
			{
				...
				Tracer<JenesisAdaptation>.WriteError(ex);
				if (flag)
				{
					throw;
				}
			...
			{
				Tracer<JenesisAdaptation>.LogExit("DownloadPackage");
			}
		}
		...
		{
			Tracer<JenesisAdaptation>.LogEntry("FlashDevice");
			...
			Tracer<JenesisAdaptation>.LogExit("FlashDevice");
		}
		...
		{
			Phone phone = this.GetPhone(JenesisModels.WPJ40_10);
			phone.QueryParameters = JenesisMsrQuery.WPJ40_10;
		...
		protected override Stream GetImageDataStream(Phone phone)
		{
			...
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("WPJ40-10_image_set1.png"));
		...
		protected override Stream GetManufacturerImageDataStream()
		{
			Assembly executingAssembly = Assembly.GetExecutingAssembly();
			string[] manifestResourceNames = executingAssembly.GetManifestResourceNames();
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("jenesis_logo.png"));
			if (!string.IsNullOrEmpty(text))
			{
				return executingAssembly.GetManifestResourceStream(text);
			}
		...
		{
			Tracer<JenesisAdaptation>.LogEntry("GetFfuDevice");
			FFUManager.Start();
			...
			{
				FFUManager.Stop();
				Tracer<JenesisAdaptation>.LogExit("GetFfuDevice");
			}
