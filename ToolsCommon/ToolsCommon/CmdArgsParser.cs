				...
				while (enumerator2.MoveNext())
				{
					...
					if (type2.IsGenericType)
					{
						if (type2.GetGenericTypeDefinition() == typeof(List<>))
						{
							...
							Type contentType = type3;
							KeyValuePair<string, string> commandEntry2 = commandEntry;
							IList value = CmdArgsParser.ReflectionListFactory(contentType, commandEntry2.Value);
							property.SetValue(t, value, null);
						}
						else if (type2.GetGenericTypeDefinition() == typeof(Dictionary<, >))
						{
							Type type4 = type2.GetGenericArguments()[0];
							Type type5 = type2.GetGenericArguments()[1];
							Type keyType = type4;
							Type valueType = type5;
							KeyValuePair<string, string> commandEntry3 = commandEntry;
							IDictionary value2 = CmdArgsParser.ReflectionDictionaryFactory(keyType, valueType, commandEntry3.Value);
						...
						else
						{
							...
							Type type6 = type2.GetGenericArguments()[0];
							Type contentType2 = type6;
							KeyValuePair<string, string> commandEntry4 = commandEntry;
							string value3 = commandEntry4.Value;
							KeyValuePair<string, string> commandEntry5 = commandEntry;
							IEnumerable value4 = CmdArgsParser.ReflectionSetFactory(contentType2, value3, commandEntry5.Key);
							property.SetValue(t, value4, null);
						}
					}
					else if (type2.IsEnum)
					{
						...
						{
							string text2 = Enum.GetNames(type2)[0];
							foreach (string arg in Enum.GetNames(type2).Skip(1))
							{
								text2 += string.Format(" {0}", arg);
							}
							string format = "The value for \"{0}\" is incorrect.\nValue: {1}\nSupported Values: {2}";
							...
							throw new ArgumentException(string.Format(format, key, commandEntry7.Value, text2));
						}
						PropertyInfo propertyInfo = property;
						object obj2 = t;
						Type enumType = type2;
						KeyValuePair<string, string> commandEntry8 = commandEntry;
						propertyInfo.SetValue(obj2, Convert.ChangeType(Enum.Parse(enumType, commandEntry8.Value, true), type2), null);
					}
					else
					{
						...
						if (!typeConverter.IsValid(commandEntry9.Value))
						{
							...
							throw new ArgumentException(string.Format(format2, key2, commandEntry11.Value, type2.Name));
						}
			...
			foreach (XElement xelement in xdocument.Root.Elements())
			{
				...
				else
				{
					...
					if (cmds.ContainsKey(text))
					{
						...
						{
							string key;
							cmds[key = text] = cmds[key] + string.Format(";{0}", text2);
						}
					}
					else
					{
		...
		private static Dictionary<string, string> ProcessCommandLine<T>(List<string> args, List<CmdModes> modes) where T : class, new()
		{
			...
			foreach (string text in args)
			{
				...
				else
				{
					...
					{
						text2 = text.Substring(1, text.IndexOf(":") - 1);
					}
					if (text.IndexOf(":") != -1)
					{
