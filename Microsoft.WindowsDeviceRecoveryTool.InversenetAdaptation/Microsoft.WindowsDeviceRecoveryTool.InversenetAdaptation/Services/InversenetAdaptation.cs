...
namespace Microsoft.WindowsDeviceRecoveryTool.InversenetAdaptation.Services
{
	// Token: 0x02000006 RID: 6
	[PartCreationPolicy(CreationPolicy.Shared)]
	[ExportAdaptation(Type = PhoneTypes.Inversenet)]
	public class InversenetAdaptation : BaseAdaptation
	{
		// Token: 0x06000011 RID: 17 RVA: 0x00002520 File Offset: 0x00000720
		[ImportingConstructor]
		public InversenetAdaptation(FfuFileInfoService ffuFileInfoService, MsrService msrService, ReportingService reportingService)
		{
		...
		public override PackageFileInfo CheckLatestPackage(Phone phone, CancellationToken cancellationToken)
		{
			...
			try
			{
				Tracer<InversenetAdaptation>.LogEntry("CheckLatestPackage");
			...
			{
				Tracer<InversenetAdaptation>.LogExit("CheckLatestPackage");
			}
			return result;
		}
		...
		{
			Tracer<InversenetAdaptation>.LogEntry("CheckPackageIntegrity");
			this.ffuFileInfoService.ReadFfuFile(phone.PackageFilePath);
			Tracer<InversenetAdaptation>.LogExit("CheckPackageIntegrity");
		}
		...
		{
			Tracer<InversenetAdaptation>.LogEntry("DownloadPackage");
			...
			try
			{
				...
				DownloadParameters downloadParameters = new DownloadParameters
				{
					...
					DestinationFolder = ((!string.IsNullOrEmpty(phone.QueryParameters.ManufacturerHardwareModel)) ? Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetInversenetProductsPath(phone.QueryParameters.ManufacturerHardwareModel) : Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetInversenetProductsPath(phone.QueryParameters.ManufacturerHardwareVariant)),
					FilesVersioned = true
				};
				Tracer<InversenetAdaptation>.WriteInformation("Download Params: {0}", new object[]
				{
					downloadParameters
				});
			...
			catch (Exception ex)
			{
				...
				Tracer<InversenetAdaptation>.WriteError(ex);
				if (flag)
				{
					throw;
				}
			...
			{
				Tracer<InversenetAdaptation>.LogExit("DownloadPackage");
			}
		}
		...
		{
			Tracer<InversenetAdaptation>.LogEntry("FlashDevice");
			...
			Tracer<InversenetAdaptation>.LogExit("FlashDevice");
		}
		...
		{
			Phone phone = this.GetPhone(InversenetModels.EveryPhone);
			phone.QueryParameters = InversenetMsrQuery.EveryPhone;
		...
		protected override Stream GetImageDataStream(Phone phone)
		{
			...
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("EveryPhone.jpg"));
		...
		protected override Stream GetManufacturerImageDataStream()
		{
			Assembly executingAssembly = Assembly.GetExecutingAssembly();
			string[] manifestResourceNames = executingAssembly.GetManifestResourceNames();
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("Everyphone_logo.jpg"));
			if (!string.IsNullOrEmpty(text))
			{
				return executingAssembly.GetManifestResourceStream(text);
			}
		...
		{
			Tracer<InversenetAdaptation>.LogEntry("GetFfuDevice");
			FFUManager.Start();
			...
			{
				FFUManager.Stop();
				Tracer<InversenetAdaptation>.LogExit("GetFfuDevice");
			}
