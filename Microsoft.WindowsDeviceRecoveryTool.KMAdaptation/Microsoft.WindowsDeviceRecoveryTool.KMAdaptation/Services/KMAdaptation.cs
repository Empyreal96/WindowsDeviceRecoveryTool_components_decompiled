...
namespace Microsoft.WindowsDeviceRecoveryTool.KMAdaptation.Services
{
	// Token: 0x02000006 RID: 6
	[PartCreationPolicy(CreationPolicy.Shared)]
	[ExportAdaptation(Type = PhoneTypes.KM)]
	public class KMAdaptation : BaseAdaptation
	{
		// Token: 0x06000011 RID: 17 RVA: 0x00002520 File Offset: 0x00000720
		[ImportingConstructor]
		public KMAdaptation(FfuFileInfoService ffuFileInfoService, MsrService msrService, ReportingService reportingService)
		{
		...
		public override PackageFileInfo CheckLatestPackage(Phone phone, CancellationToken cancellationToken)
		{
			...
			try
			{
				Tracer<KMAdaptation>.LogEntry("CheckLatestPackage");
			...
			{
				Tracer<KMAdaptation>.LogExit("CheckLatestPackage");
			}
			return result;
		}
		...
		{
			Tracer<KMAdaptation>.LogEntry("CheckPackageIntegrity");
			this.ffuFileInfoService.ReadFfuFile(phone.PackageFilePath);
			Tracer<KMAdaptation>.LogExit("CheckPackageIntegrity");
		}
		...
		{
			Tracer<KMAdaptation>.LogEntry("DownloadPackage");
			...
			try
			{
				...
				DownloadParameters downloadParameters = new DownloadParameters
				{
					...
					DestinationFolder = ((!string.IsNullOrEmpty(phone.QueryParameters.ManufacturerHardwareModel)) ? Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetKMProductsPath(phone.QueryParameters.ManufacturerHardwareModel) : Microsoft.WindowsDeviceRecoveryTool.Model.FileSystemInfo.GetKMProductsPath(phone.QueryParameters.ManufacturerHardwareVariant)),
					FilesVersioned = true
				};
				Tracer<KMAdaptation>.WriteInformation("Download Params: {0}", new object[]
				{
					downloadParameters
				});
			...
			catch (Exception ex)
			{
				...
				Tracer<KMAdaptation>.WriteError(ex);
				if (flag)
				{
					throw;
				}
			...
			{
				Tracer<KMAdaptation>.LogExit("DownloadPackage");
			}
		}
		...
		{
			Tracer<KMAdaptation>.LogEntry("FlashDevice");
			...
			Tracer<KMAdaptation>.LogExit("FlashDevice");
		}
		...
		{
			Phone phone = this.GetPhone(KMModels.SOUL2);
			phone.QueryParameters = KMMsrQuery.Soul2;
		...
		protected override Stream GetImageDataStream(Phone phone)
		{
			...
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("Move_4.2.png"));
		...
		protected override Stream GetManufacturerImageDataStream()
		{
			Assembly executingAssembly = Assembly.GetExecutingAssembly();
			string[] manifestResourceNames = executingAssembly.GetManifestResourceNames();
			string text = manifestResourceNames.FirstOrDefault((string resourceName) => resourceName.Contains("Kruger-Matz_logo.png"));
			if (!string.IsNullOrEmpty(text))
			{
				return executingAssembly.GetManifestResourceStream(text);
			}
		...
		{
			Tracer<KMAdaptation>.LogEntry("GetFfuDevice");
			FFUManager.Start();
			...
			{
				FFUManager.Stop();
				Tracer<KMAdaptation>.LogExit("GetFfuDevice");
			}
